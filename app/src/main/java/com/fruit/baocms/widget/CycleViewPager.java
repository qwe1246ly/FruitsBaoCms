...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
	...
	public void setData(List<ImageView> views, List<String> list, ImageCycleViewListener listener, int showPosition) {
		...
		for (int i = 0; i < indicators.length; i++) {
			...
			params.setMargins(5, 0, 5, 0);                //  设置指示器 间距
	...
	private void setIndicator(int selectedPosition) {
		...
		//	indicators[i].setBackgroundResource(R.mipmap.icon_point);
			indicators[i].setImageResource(R.drawable.point_focured);
		}
		if (indicators.length > selectedPosition)

			indicators[selectedPosition].setImageResource(R.drawable.point_nomal);
